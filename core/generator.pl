...
scheduleacc([(Course,K,Duration)|Rest], Partial, Final) :-
    ...
    check_all_timeslots(Course, Room, Timeslots, Partial),
    add_sessions(Course, K, Room, Timeslots, Partial, NewPartial),
    scheduleacc(Rest, NewPartial, Final).

check_all_timeslots(_, _, [], _).
check_all_timeslots(Course, Room, [TS|Rest], Partial) :-
    instructor_availability_ok(Course, TS),
    no_room_conflict(Room, TS, Partial),
    no_course_conflict(Course, TS, Partial),
    no_group_conflict(Course, TS, Partial),
    no_fatigue_conflict(Course, TS, Partial),
    group_availability_ok(Course, TS),
    check_all_timeslots(Course, Room, Rest, Partial).

add_sessions(_, _, _, [], Partial, Partial).
add_sessions(Course, K, Room, [TS|Rest], Partial, [session(Course, K, Room, TS)|NewPartial]) :-
    add_sessions(Course, K, Room, Rest, Partial, NewPartial).
